...
TR PV_Count(Rain Inp)
{
	...
	while (i < Inp.PVn - 3)
	{
		if (PV_4Points(*(Inp.Value + i), *(Inp.Value + i + 1), *(Inp.Value + i + 2), *(Inp.Value + i + 3)))
		{
			...
			*(ave + i) = 0;
			*(ave + i + 1) = 0;
			*(ave + i + 2) = *(Inp.Value + i + 1) + *(Inp.Value + i + 2);

			//幅值
			*(amp + i) = 0;
			*(amp + i + 1) = 0;
			*(amp + i + 2) = abs(*(Inp.Value + i + 1) - *(Inp.Value + i + 2));
	...
	else
	{
		...
		if (i == Inp.PVn - 2)
		{
			if (PV_4Points(*(Inp.Value + i), *(Inp.Value + i + 1), *(Inp.Value), *(Inp.Value + 1)))
			{
				*(ave + i) = 0;
				*(ave + i + 1) = 0;
				*(ave + i + 2) = *(Inp.Value + i + 1) + *(Inp.Value);
				
				*(amp + i) = 0;
				*(amp + i + 1) = 0;
				*(amp + i + 2) = abs(*(Inp.Value + i + 1) - *(Inp.Value));
...
)
{
	...
	DataOk a = PV_NoSame(RawData, LenRawData);//剔除重复数据
